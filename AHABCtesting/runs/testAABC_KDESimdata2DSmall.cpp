...
* 
* Just does kde results, for comparison with histgrams
...
void runAABC_Simdata()
{
	...
	int myPid = getpid();
	std::cout << "Simdata aabc kde 2d small" << std::endl;
	std::cout << "This process id is " << myPid << std::endl;
	
	
	std::string baseOutputDir("../figures/");
	
	std::string thisDir("AABC_KDESimData2DSmall");
	
	std::string descriptionString("AABC_KDEsimdata2DGR");
	...
	size_t iterations = 1000; // number of joint observations to generate
	...
	priorLims.push_back( pair<double, double>
			...
			(0.50*4*Nzero/unit, 10.0*4*Nzero/unit));
	...
	doKDE_AABCSimsMultLociPolytableHeterozygosity(prefix,
				spec,
				r,
				iterations,
				n_obs,
