...
void runAABC_Simdata()
{
	...
	int myPid = getpid();
	std::cout << "Simdata aabc kde 4d small" << std::endl;
	std::cout << "This process id is " << myPid << std::endl;
	
	
	std::string baseOutputDir("../figures/");
	
	std::string thisDir("AABC_KDESimData4DSmall");
	
	std::string descriptionString("AABC_KDEsimdata4DGR");
	...
	size_t iterations = 2000; // number of joint observations to generate
	...
	doKDE_AABCSimsMultLociPolytableHeterozygosityAndSegSites(prefix,
				spec,
				r,
				iterations,
				n_obs,
