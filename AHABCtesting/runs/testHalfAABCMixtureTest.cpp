...
/*! \file
\brief Do AABC - RPABC runs using MixtureTest.
* 
One AABC iteration followed by slice etc
* 
*/
...
void runAABC_Mixture()
{
	...
	int myPid = getpid();
	std::cout << "MixtureTest aabc" << std::endl;
	std::cout << "This process id is " << myPid << std::endl;
	
	std::string baseOutputDir("../figures/");
	
	std::string thisDir("HalfAABCMixture2DLargerNewSigma");
	
	std::string descriptionString("HalfAABCMixture2DGR");
				...
				bool doIndividual)
{
	...
	boost::shared_ptr < PopMCabcSimulatorAdaptive > simulatorPtr
							 = doSimulationMixture(
									r_loops,
									n_reps_per_particle*n_obs, // take account of averaging	
									priorLims,
									allRefObjsPtr,
									iterations,
									baseEpsilonPercentile,
				...
				const std::string& logfilename) 
{
	...
	boost::shared_ptr < GenericDataObjectSampler > sampler ( new MixtureTestSampler(rr_sampler) );
