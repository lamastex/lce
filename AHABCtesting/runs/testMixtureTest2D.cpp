...
/*! \file
\brief Try to get some sort of histogram for data using Beaumont etc toy model in 2D
...
//#define MINVOLMAIN
#define MINDEPTHMAIN 25
#define MINVOLPRED
#define MINDEPTHPRED 25
...
{
	int myPid = getpid();
	std::cout << "basic ahabc test for mixture test 2 d with small prior range" << std::endl;
	std::cout << "This process id is " << myPid << std::endl;
	
	std::string baseOutputDir("../figures/");
	
	std::string thisDir("MixtureTest4DmvPred25New");
	#ifdef REDUCE_PRIOR_LIMS
		thisDir += "Rdcd";
	#endif
	
	std::string descriptionString("Mixture4DGR");
	...
	size_t n_reps = 500000; // number of joint observations to generate
	...
	std::vector < std::vector < double > > otherParams;
	{
			...
			double pml = priorLims.back().first;
			double pmt = priorLims.back().second;
			double m1 = 1.0;
			double m2 = 0.0;
			double m3 = 2.0;
			if (m1 < pml) m1 = pml;
