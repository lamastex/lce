...
/*! \file
\brief Tsting of basic testing of AHABC using Simdata sampler, 
...
{
	int myPid = getpid();
	std::cout << "testing routine for basic ahabc for simdata 2d with small prior range" << std::endl;
	std::cout << "This process id is " << myPid << std::endl;
	
	std::string baseOutputDir("../figures/");
	
	std::string thisDir("SimData2DSmallTesting");
	...
	size_t n_reps = 500; // number of joint observations to generate
	...
	std::vector < std::vector < double > > otherParams;
	{
		...
		{
			double t = 4.0*4*Nzero/unit;
			if (!(t > ptl)) t = ptl + (theta_per_site - ptl)*0.8;
			assert(t > 0.0); 
			otherParams.push_back(std::vector < double >(1, t));
		}
		{
			double t = 6.0*4*Nzero/unit;
			if (!(t < ptt)) t = theta_per_site + (ptt - theta_per_site)*0.05;
			assert(t > 0.0); 
			otherParams.push_back(std::vector < double >(1, t));
		}
	}
