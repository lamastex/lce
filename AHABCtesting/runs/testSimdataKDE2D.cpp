...
1 parameter, 1 summary stat, large prior range,
...
{
	int myPid = getpid();
	std::cout << "ahabc with kde function approximation for simdata 2d with large prior range" << std::endl;
	std::cout << "This process id is " << myPid << std::endl;
	
	std::string baseOutputDir("../figures/");
	
	std::string thisDir("SimDataKDE2DNewParams");
	#ifdef REDUCE_PRIOR_LIMS
		thisDir += "Rdcd";
	#endif
	
	std::string descriptionString("simdata2DKDE");
	...
	cxsc::real maxMeasure(0.00001);// to use for approx to kde
	...
	otherParams.push_back(std::vector < double >(1, 4.0*4*Nzero/unit));
	otherParams.push_back(std::vector < double >(1, 6.0*4*Nzero/unit));
