    ...
    */
    ...
    for (unsigned CNTR=0; CNTR < p.NumOfSfsData; CNTR++)
    {
        ...
        */
        ...
        int Exact_pvalue = 0;
        int ExactUnc_pvalue = 0;
        int MAXNTRIES = 250000;
...
//        make_set_of_pees_tees(rgsl, PEES, TEES, growth_rate, p);
        ...
        for (int tries = 1; tries <= MAXNTRIES; tries++)
          {                 // try a bunch of times
            ...
            double probsimUnc = Multinomial_Prob_Ln(p.SfsSz, &lineageLenItonsUnc[0],
                                               (unsigned *)(const_cast<int *> (&vaSFS[0]))); 
            double probobsUnc = Multinomial_Prob_Ln(p.SfsSz, &lineageLenItonsUnc[0],  
                                               (unsigned *)(const_cast<int *> (&obsSfs[0]))); 
            if(probsimUnc <= probobsUnc) ExactUnc_pvalue++;
            ...
            // DO NOW: Need to have a Dirichlet Posterior shaving of EpochTimes*f
            // OR look in the sfs-sieve conditioned on S and D of some radius...
            // OR folded SFS - much easier... 
            double probsim = Multinomial_Prob_Ln(p.SfsSz, &lineageLenItons[0],
                                               (unsigned *)(const_cast<int *> (&vaSFS[0]))); 
            double probobs = Multinomial_Prob_Ln(p.SfsSz, &lineageLenItons[0],  
                                               (unsigned *)(const_cast<int *> (&obsSfs[0]))); 
            //cout << probsim << "\t" << probobs << endl; getchar();
            if(probsim <= probobs) Exact_pvalue++;
        ...
        double Exact_p = ((double)(Exact_pvalue))/(double(MAXNTRIES));
        double ExactUnc_p = ((double)(ExactUnc_pvalue))/(double(MAXNTRIES));
        //cout << "TajimasD_pvalues Un/controlled = \n";
        cout << SFSI.S << "\t" << SFSI.Pi << "\t" << SFSI.D << "\t"
             << TajimasDUnc_2p << "\t" << TajimasD_2p << "\t"
             << TajimasDUnc_p << "\t" << TajimasD_p << "\t"
             << ExactUnc_p << "\t" << Exact_p << endl;
