/*! \file TajDPvals1.cpp
    \brief main function for p-values from Tajima's D statistics >1 Locus at a time (more RAM)
*/
    ...
    */
    ...
    for (unsigned CNTR=0; CNTR < p.NumOfSfsData; CNTR++)
    {
        ...
        */
        ...
        int TajimasD_1gtpvalue = 0;
        int TajimasDUnc_1gtpvalue = 0;
        int TajimasD_1ltpvalue = 0;
        int TajimasDUnc_1ltpvalue = 0;
...
//        make_set_of_pees_tees(rgsl, PEES, TEES, growth_rate, p);
        ...
        for (int tries = 1; tries <= MAXNTRIES; tries++)
          {                 // try a bunch of times
            ...
            if (SFSIsimUnc.D <= SFSI.D) { TajimasDUnc_1ltpvalue++; }
            if (SFSIsimUnc.D >= SFSI.D) { TajimasDUnc_1gtpvalue++; }
            ...
            if (SFSIsim.D <= SFSI.D) { TajimasD_1ltpvalue++; }
            if (SFSIsim.D >= SFSI.D) { TajimasD_1gtpvalue++; }
        ...
        double TajimasD_1ltp = ((double)(TajimasD_1ltpvalue))/(double(MAXNTRIES));
        double TajimasDUnc_1ltp = ((double)(TajimasDUnc_1ltpvalue))/(double(MAXNTRIES));
        double TajimasD_1gtp = ((double)(TajimasD_1gtpvalue))/(double(MAXNTRIES));
        double TajimasDUnc_1gtp = ((double)(TajimasDUnc_1gtpvalue))/(double(MAXNTRIES));
        //cout << "TajimasD_pvalues Un/controlled = \n";
        cout << (CNTR+1) << "\t" << K << "\t" << accKCProb << "\t"
             << SFSI.S << "\t" << SFSI.Pi << "\t" << SFSI.D << "\t"
             << TajimasDUnc_2p << "\t" << TajimasD_2p << "\t"
             << TajimasDUnc_p << "\t" << TajimasD_p << "\t"
             << TajimasDUnc_1ltp << "\t" << TajimasD_1ltp << "\t"
             << TajimasDUnc_1gtp << "\t" << TajimasD_1gtp << endl;
