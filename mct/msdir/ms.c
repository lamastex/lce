...
gensam( char **list, double *pprobss, double *ptmrca, double *pttot ) 
{
    ...
    if( (segsitesin == 0) && ( theta > 0.0)   ) {
	  ...
	  for( seg=0, k=0; k<nsegs; seg=seglst[seg].next, k++) { 
		...
		#ifdef MYDEBUG
			printf("back in gensam, segsit = %d\n", segsit);
		#endif
	...
	double beg, len, *ptr;
{
	...
	#ifdef MYDEBUG
		printf("\n\tback in locate, completing new positions\n");
	#endif
	
	for(i=0; i<n; i++) {
		
		#ifdef MYDEBUG
			printf("\t\ti = %d, ptr[i] = %f, ptr[i] = beg + ptr[i]*len = %f\n", i, ptr[i], beg + ptr[i]*len);
		#endif
		ptr[i] = beg + ptr[i]*len ;
	}
...
make_gametes(int nsam, int mfreq, struct node *ptree, double tt, int newsites, int ns, char **list )
{
	...
	#ifdef MYDEBUG
			printf("\nin make gametes, newsites = %d, tt = %f\n", newsites, tt);
	#endif

	for(  j=ns; j< ns+newsites ;  j++ ) {
		#ifdef MYDEBUG
			printf("\tj = %d, about to pick node\n", j);
		#endif
		...
		#ifdef MYDEBUG
			printf("\n\tback in make gametes with j = %d, node = %d\n", j, node);
		#endif
	...
	double tt;
{
	...
	for( i=0, y=0; i < 2*nsam-2 ; i++) {
		...
		if( y >= x ) {
			#ifdef MYDEBUG
				printf("\ty >= x, returning i = %d\n", i);
			#endif
			return( i ) ;
		}
	}
	#ifdef MYDEBUG
		printf("\treturning 2*nsam - 3 = %d\n", 2*nsam - 3);
	#endif
	...
	double pbuf[];
{
	...
	#ifdef MYDEBUG
		printf("\n\tending ordran, after order(n,pbuf), new bits of pbuf are:\n");
		int i;
		for(i=0; i<n; i++) {
			printf("\t(i = %d, pbuf[i] = %f)\n", i, pbuf[i]);
	...
	double u;
{
	...
	if( u > 30. ){
		...
		#ifdef MYDEBUG
			printf("\tu > 30, i = (int)(0.5 + gasdev(u,u)) = %d\n", i);
			if (i < 0) printf("\ti < 0, returning 0\n");
			else printf("\ti >= 0, returning %d\n",i);
		#endif
	    if( i < 0 ) return( 0 ) ;
	    else return( i ) ;
	  }
	...
	#ifdef MYDEBUG
		printf("\tu <= 30, ru = %f, p = exp(-u) = %f\n", ru, p);
		if (ru < p) printf("\tru < p, returning 0\n");
	#endif
	if( ru < p) return(0);
	cump = p;
	
	#ifdef MYDEBUG
		printf("\tru >= p, i = %d, cump = %f\n", i, cump);
	#endif
	...
	double m, v;
{
	...
	if  (iset == 0) {
		...
		#ifdef MYDEBUG
			printf("\t\troutine for iset = 0, v1 = %f, v2 = %f\n", v1, v2);
			printf("\t\tfac = %f, gset = %f\n", fac, gset);
			printf("\t\treturning m + sqrt(v)*v2*fac = %f\n", m + sqrt(v)*v2*fac);
		#endif
		...
		#ifdef MYDEBUG
			printf("\t\troutine for iset = 1\n");
			printf("\t\tgset = %f\n", gset);
			printf("\t\treturning m + sqrt(v)*gset = %f\n", m + sqrt(v)*gset);
		#endif
		iset=0;
