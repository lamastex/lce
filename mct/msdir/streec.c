...
/* Main loop */
	...
	while( nchrom > 1 ) {
		...
		for( i=0; i<npop; i++) {
			...
			#ifdef MYDEBUG
				printf("i = %d\tconfig[i] = %d\tmig[i][i] = %f\tconfig[i]*migm[i][i] = %f\n",i,config[i],migm[i][i],config[i]*migm[i][i]);
				printf("mig = %f\n",mig);
			#endif
		}
		...
		coal_prob = ((double)config[pop])*(config[pop]-1.) ;
		   ...
		   while( ( rdum = ran1() )  == .0 ) ;
			 ...
			 ttemp = -log( rdum )*size[pop] /coal_prob ;
			    ...
			    cpop = pop;
			#ifdef MYDEBUG
				printf("\ncoalescent event type 1 created\n");
				printf("coal_prob = %f,\trdum = %f\n", coal_prob,rdum);
				printf("cpop = %d\n", cpop);
				printf("ttime = %f\n", tmin);
		     ...
		     if( arg > 0.0 ) {                          /*if arg <= 0,  no coalescent within interval */ 
			    ...
			    cpop = pop ;
			    			#ifdef MYDEBUG
				printf("\ncoalescent event type 2 created\n");
				printf("coal_prob = %f,\trdum = %f\n", coal_prob,rdum);
				printf("cpop = %d\n", cpop);
				printf("ttime = %f\n", tmin);
	...
	if( ( ( eflag == 0) && (nextevent != NULL))|| ( (nextevent != NULL) &&  ( (t+tmin) >=  nextevent->time)) ) {
	    t = nextevent->time ;
	    // JAH change 15/12/2011 to make all demographic events scheduled at the same
	    // time be processed before going back into loop
	    while ( nextevent != NULL && nextevent->time == t) {
			switch(  nextevent->detype ) {
			case 'N' :
			
				#ifdef MYDEBUG
					printf("\nevent -eN %f %f\n", nextevent->time, nextevent->paramv);
					
				#endif
			   for(pop =0; pop <npop; pop++){
				 #ifdef MYDEBUG
					printf("pop = %d, current size = %f\n", pop, size[pop]);
			...
			case 'n' :
				#ifdef MYDEBUG
					printf("\nevent -en %f %d %f\n", nextevent->time, nextevent->popi, nextevent->paramv);
			...
			case 'm' :
			  #ifdef MYDEBUG
					printf("\nevent -em %f %d % d %f\n", nextevent->time, nextevent->popi, nextevent->popj, nextevent->paramv);
			  ...
			  migm[i][i] += nextevent->paramv - migm[i][j];
			   ...
			   break;
				...
				#ifdef MYDEBUG
					printf("\nevent -ej %f %d %d\n", nextevent->time, nextevent->popi, nextevent->popj);
		...
		} //end while all events at same time
 	   } 
	else {
			#ifdef MYDEBUG
				printf("about to process event, type is = %c and min is %f",event, tmin);
			#endif
...
xover(int nsam,int ic, int is)
{
   ...
   /* copy last part of chrom to nchrom  */
	...
	#ifdef MYDEBUG
		printf("and copied in segments, size for these segments is %d\n",newsg);
		printf("and links for these segments =  %d\n", links(nchrom-1));
	#endif
	...
	#ifdef MYDEBUG
		printf("nlinks =  %d\n", nlinks);
		printf("taking off links for change = %d will leave\n", pseg2->beg - (pseg+lsgm1)->end);
...
if( !(chrom[ic].pseg = 
     (struct seg *)realloc(chrom[ic].pseg,(unsigned)(lsg*sizeof(struct seg)) )) )
		perror( " realloc error. re2");
			...
			#ifdef MYDEBUG
		printf("erased the copied segmetns, size for this chromosones segments = %d\n",lsg);
		printf("and links for these segments =  %d\n", links(ic));
	#endif
	...
	if( in ) {
		...
		for( i=0,k=0; (k<nsegs-1)&&(begs > seglst[seglst[i].next].beg-1);
		   i=seglst[i].next, k++) ;
		   		#ifdef MYDEBUG
			printf("checking for new tree: begs = %d\ti = %d\tseglst[i].beg = %d\n",begs, i, seglst[i].beg);
		#endif
 
		if( begs != seglst[i].beg ) {
						/* new tree  */
			#ifdef MYDEBUG
				printf("adding new tree\n");
			#endif
	...
	int  nsites;
{
	...
	for( seg=0, k=0; k<nsegs; seg=seglst[seg].next, k++) {
		...
		yes2 = isseg(start, c2, &seg2);
				#ifdef MYDEBUG
			printf("k = %d\t\tseg = %d\n",k, seg);
			printf("seg1 = %d\t\tyes1 = %d\n", seg1, yes1);
			printf("seg2 = %d\t\tyes2 = %d\n", seg2, yes2);
		#endif

		if( yes1 || yes2 ) {
			tseg++;
						#ifdef MYDEBUG
				printf("tseg is now = %d\n",tseg);
			#endif
			...
			#ifdef MYDEBUG
				printf("setting end for new seg at tseg to %d\n", end);
			#endif
			...
			if( yes1 && yes2 ) {
				...
				#ifdef MYDEBUG
					printf("adding a node: nnodes[%d] is now %d\n",seg,nnodes[seg]);
				#endif
	...
	#ifdef MYDEBUG
		printf("nlinks =  %d\n", nlinks);
		printf("taking off links for c1 = %d will leave\n", links(c1));
		printf("nlinks = %d\n",nlinks - links(c1));
	#endif
	...
	if( tseg < 0 ) {
		...
		#ifdef MYDEBUG
			printf("nlinks =  %d\n", nlinks);
			printf("adding on links for c1 = %d will give\n", links(c1));
			printf("nlinks = %d\n",nlinks + links(c1));
		#endif
	...
	#ifdef MYDEBUG
		printf("nlinks =  %d\n", nlinks);
		printf("taking off links for c2 = %d will leave\n", links(c2));
		printf("nlinks = %d\n",nlinks - links(c2));
	#endif
	nlinks -= links(c2);
