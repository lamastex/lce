...
/*! \file
\brief Run AABC using a mct::MultiLociSimDataSampler.
* 
...
In this run the observed data is randomly simulated using the
a mct::MultiLociSimDataSampler.
...
void runAABC_Simdata()
{
	try {
		...
		double growth = 20.0; // growth rate
		 ...
		 * use this as a sample to get some idea of how 
		 * 'typical' the chosen ref stats are */
		 ...
		 * get a sample for the reference object (ie from 'observed' data)
		 * using the true parameters 
		 * - I use the first one of these as the reference object
		 * for this aabc run but I use the whole sample to get a feel for
		 * how 'typical' this is */
		boost::shared_ptr < mct::MultiLociPolyTableSet > 
				refset = true_sampler->sample(n_examples, pop, nloci_nsites, true_p);
		
		/* The reference set should contain n_example objects
		 * and we just take the first one to get a individual reference 
		 * object for this aabc run */
		const boost::shared_ptr < const mct::MultiLociPolyTable > ref = refset->at(0);
