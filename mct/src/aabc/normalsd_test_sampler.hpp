...
/*! \file     
\brief NormalSDTestSampler declarations.
*/



#ifndef _NORMAL_SD_TEST_SAMPLER_H
#define _NORMAL_SD_TEST_SAMPLER_H
...
namespace aabc {
	...
	/** @brief A class capable of 
	 generating a sample of NormalSDTest tests.
	 
	 */

	class NormalSDTestSampler : public GenericDataObjectSampler {
		public:
			 ...
			 */
			explicit NormalSDTestSampler(
					const boost::shared_ptr < mct::PRNGen >& _r_ptr);
		
		
			virtual ~NormalSDTestSampler();
		...
		protected:
			...
			virtual boost::shared_ptr < NormalSDTest > genNew(
