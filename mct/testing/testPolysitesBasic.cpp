...
/*! \file
\brief Basic testing of polysites sampler.
...
// basic test that polysites sampling is working
void polysitesTesting()
{
	try {
		int myPid = getpid();
		...
		//mutation parameter PER SITE ie 4N0(mu/site)
		...
		for (std::vector < double>::iterator theta_it = thetas.begin(); 
			...
			for (std::vector < double>::iterator growth_it = growths.begin(); 
				...
				std::string filenamepop = "TestPolysites";
