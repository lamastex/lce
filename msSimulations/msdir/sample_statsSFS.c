	...
	char *argv[];
{
	...
	FILE *pf, *fopen(), *pfin ;
	double *posit   ;
        int *sf, *sfs ; // Raaz added this for SF and SFS
...
/* read in first two lines of output  (parameters and seed) */
  ...
  sf = (int *)malloc( maxsites*sizeof( int ) ) ;
  sfs = (int *)malloc( nsam*sizeof( int ) ) ;
...
/* analyse sample ( do stuff with segsites and list) */
	...
	else // raaz commented next line and added the following few lines for site frequencies output
	  //printf("pi:\t%lf\tss:\t%d\tD:\t%lf\tthetaH:\t%lf\tH:\t%lf%s", pi,segsites, tajd(nsam,segsites,pi) , th , h,slashline  ) ;

          //compute and print site frequencies or SF
	  for( int i=0; i<segsites-1 ; i++) {
              sf[i]= frequency('1', i, nsam,list); //printf("%d ", sf[i]); 
          }
          int i= segsites-1; sf[i] = frequency('1', i, nsam,list); //printf("%d%s",  sf[i], slashline );

	  // compute and print SFS
          for (int j=0; j<nsam; j++) sfs[j]=0; //initialize sfs
	  for( int i=0; i<segsites ; i++)  
            for (int j=0; j<nsam; j++) if (sf[i]==j+1) sfs[j]=sfs[j]+1;
          for (int j=0; j<nsam-2; j++) printf("%d ", sfs[j]); printf("%d%s",  sfs[nsam-2], slashline );
...
{
	int s; //, frequency( char, int, int, char**);// Raaz made this a function signature on top
