...
/* 	      } */
/* 	  } */
/* 	for( i = 1 ; i < pars.mp.ntheta ; ++i ) */
/* 	  { */
/* 	  } */
	pars.mp.theta = pars.mp.thetas[0];
      }
    if( !pars.commandlineseedflag ) seedit( "end" );
}
...
{
  int nsegs, h, i, k, j, seg, ns, start, end, len, segsit,dummy ;
...
/*     if( (segsitesin == 0) && ( theta > 0.0)   )  */
/*       { */
	for(dummy=0;dummy<pars.mp.ntheta;++dummy)
	  {
	    theta = pars.mp.thetas[dummy];
	    	    if( (segsitesin == 0) && ( theta > 0.0)   ) 
	      {
		ns = 0 ;
		for( seg=0, k=0; k<nsegs; seg=seglst[seg].next, k++) { 
	    ...
	    if( ns > 0 )	fprintf(stdout,"\npositions: ");
	    for( i=0; i<ns; i++)
	      fprintf(stdout,"%6.4lf ",posit[i] );
	    fprintf(stdout,"\n");
	    if( ns > 0 )
	...
	*phowmany = atoi( argv[2] );
	...
	while( arg < argc ){
		...
		switch ( argv[arg][1] ){
			...
			case 't' : 
				arg++;
				pars.mp.ntheta = atoi(argv[arg++]);
				pars.mp.theta = atof(  argv[arg] );
				pars.mp.thetas = (double *)malloc(pars.mp.ntheta*sizeof(double));
				for( i=0 ; i < pars.mp.ntheta ; ++i )
				  {
				    pars.mp.thetas[i] = atof(argv[arg++]);
				  }
				//argcheck( arg, argc, argv);
				break;
			case 's' : 
